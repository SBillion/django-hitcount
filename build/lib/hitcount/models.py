...
def delete_hit_count_callback(sender, instance, save_hitcount=False, **kwargs):
    """
...
class HitCount(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and self.object_pk and self.content_type:
            ...
            if HitCount.objects.filter(object_pk=self.object_pk).filter(
                    content_type=self.content_type):
                raise (DuplicateContentObject, "A HitCount object already " + "exists for this " \
                                                                             "content_object.")
    ...
    def get_content_object_url(self):
        ...
        '''
        self.get_content_object_target().get_absolute_url()

    def get_content_object_target(self):
        '''
        Django has this in its contrib.comments.model file -- seems worth
        implementing though it may take a couple steps.
        '''
